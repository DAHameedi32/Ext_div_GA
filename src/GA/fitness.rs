...
pub fn rank_pop(
    ...
    for j in 1..k_forms.len() {
        ...
        ranked_pop.push(compute_fitness(k_forms[j].as_ref(), k_forms[j-1].as_ref(), k_plus_one_forms[j].as_ref(), ext_d_population[j].as_ref()));
...
pub fn compute_fitness(k_form_a : MatRef<f64>,
                         k_form_b: MatRef<f64>, 
